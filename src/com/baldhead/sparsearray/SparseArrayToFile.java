...
public class SparseArrayToFile {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("当前的稀疏数组：");
       for (int i = 0;  i < sparseArr.length; i++) {
                System.out.printf("%d\t%d\t%d\t\n", sparseArr[i][0], sparseArr[i][1], sparseArr[i][2]);
        }

        //将稀疏数组存入文件系统
        boolean result = ReadAndSaveSparseArrayToFile.saveSparseArrayToFile(sparseArr);
        //存入成功之后才读取文件还原二维数组
        if (result){
            /**
             * 将稀疏数组转换为二维数组
             */
            int[][] sparseArrayToFile = ReadAndSaveSparseArrayToFile.readSparseArrayToFile();
            System.out.println("还原后的二维数组如下：");
            int reChessArr[][] = new int[sparseArrayToFile[0][0]][sparseArrayToFile[0][1]];
